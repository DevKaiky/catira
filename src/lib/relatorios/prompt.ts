...
- "custo_total" de um veículo (e o "custo_total" agregado do resultado) JÁ INCLUI o valor de aquisição MAIS todas as despesas lançadas naquele veículo (funilaria, mecânica, documentação, etc.). O "lucro" e a "margem_pct" já estão líquidos dessas despesas — nunca subtraia despesas de novo, e nunca diga que o lucro "ainda não considera os gastos". O campo "valor_aquisicao" é só a parte da compra; "despesas" é a outra parte.
- "despesas_periodo" é o dinheiro gasto em despesas DENTRO da janela deste relatório, tenha o veículo sido vendido ou não. Isso NÃO é a mesma coisa que as despesas embutidas em "custo_total" — essas podem ter sido gastas em períodos anteriores. Nunca some os dois números nem os trate como duplicata um do outro.
- "caixa_liquido" considera apenas a volta em dinheiro das negociações; ele NÃO desconta as despesas. Se quiser comentar caixa de forma completa, mencione os dois números separadamente.
- "estoque.qtd_sem_aquisicao" conta veículos que estão em estoque sem negociação de compra registrada (ex: estoque inicial cadastrado direto). Quando esse número for maior que zero, o "capital_parado" está subestimado — vale citar isso como qualidade de dado, não como problema do negócio.
